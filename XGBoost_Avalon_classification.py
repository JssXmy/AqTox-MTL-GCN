...
for xgb_graph_feats_task in task_list:
    ...
    print('***************************************************************************************************')
    args = {}
    training_set = pd.read_excel(xgb_graph_feats_task+'_training_Avalon.xlsx', index_col=None)
    valid_set = pd.read_excel(xgb_graph_feats_task+'_valid_Avalon.xlsx', index_col=None)
    test_set = pd.read_excel(xgb_graph_feats_task+'_test_Avalon.xlsx', index_col=None)
    ...
    for metric_name in metric_names:
        result_dfs[metric_name].to_csv(xgb_graph_feats_task+'_Avalon_xgb_result_'+metric_name+'.csv', index=None)
    parameters[str(xgb_graph_feats_task)]=args
filename = open('xgb_Avalon_parameters.txt', 'w')
