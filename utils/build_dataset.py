...
def multi_task_build_dataset(dataset_smiles, labels_list, smiles_name):
    ...
    split_index = dataset_smiles['group'] if 'group' in dataset_smiles.columns else ['predict']*len(dataset_smiles)
    smilesList = dataset_smiles[smiles_name]
...
def built_data_and_save_for_splited(
        origin_path='../data/AlaGroErC50.csv',
        save_path='../data/AlaGroErC50.bin',
        group_path='../data/AlaGroErC50_group.csv',
...
def load_graph_from_csv_bin_for_splited(
    ...
    for index, group_index in enumerate(group):
        if group_index=='train' and index not in not_use_index:
...
#################################预测数据集生成#####################################
def build_and_save_predicted_data(origin_path, save_path, task_list=None):
    """生成与训练格式完全兼容的预测数据集"""
    # 读取数据并验证
    data_origin = pd.read_csv(origin_path)
    if 'smiles' not in data_origin.columns:
        raise KeyError("CSV文件必须包含'smiles'列")

    # 自动处理标签列
    # 自动检测任务列
    if task_list is None:
        labels_list = [col for col in data_origin.columns if col not in ['smiles']]
    else:
        labels_list = task_list

    for col in labels_list:
        if col not in data_origin.columns:
            data_origin[col] = 0.0  # 创建全0标签列
        data_origin[col] = data_origin[col].fillna(0.0)

    # 生成伪分组信息
    data_origin['group'] = 'predict'

    # 构建数据集
    data_set_gnn = multi_task_build_dataset(
    ...
    # 提取并转换数据
    smiles, graphs, labels, mask, _ = map(list, zip(*data_set_gnn))
    
    # 转换为PyTorch张量并确保维度正确
    labels_tensor = torch.tensor(labels, dtype=torch.float32)
    mask_tensor = torch.ones_like(labels_tensor)
    
    # 保存数据（关键修复：移除smiles字段，使用独立文件保存）
    ...
    df = pd.DataFrame({
        'smiles': smiles,
        'task_names': [','.join(labels_list)]*len(smiles)  # 所有行记录任务列表
    })
    df.to_csv(save_path.replace('.bin', '_smiles.csv'), index=False)
    
    print(f"成功生成标准格式数据集: {save_path}")
    
def load_graph_for_predict(bin_path, input_csv_path, select_task_names=None):
    """健壮的预测数据加载函数（支持任务名称选择）"""
    # 加载数据
    graphs, label_dict = load_graphs(bin_path)  # 获取标签字典
    labels = label_dict['labels']
    mask = label_dict['mask']
    
    smiles_df = pd.read_csv(input_csv_path)
    original_tasks = [col for col in smiles_df.columns if col != 'smiles']  # 直接从DataFrame获取任务列
    
    # 初始化任务索引
    task_indices = []
    if select_task_names is not None:
        # 验证任务存在性
        missing_tasks = [name for name in select_task_names if name not in original_tasks]
        if missing_tasks:
            raise ValueError(f"任务名称错误: {missing_tasks} 不存在，可用任务: {original_tasks}")
        task_indices = [original_tasks.index(name) for name in select_task_names]
        
        # 筛选指定任务
        labels = labels[:, task_indices]
        mask = mask[:, task_indices]

    graphs = [g.to('cuda') for g in graphs]

    # 构建数据集
    return [
        (smiles_df.iloc[i]['smiles'], graphs[i], 
         labels[i].numpy(), mask[i].numpy())
