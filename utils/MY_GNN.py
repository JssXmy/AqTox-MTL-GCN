...
from sklearn.metrics import roc_auc_score, mean_squared_error, precision_recall_curve, auc,\
     balanced_accuracy_score, r2_score, mean_absolute_error, accuracy_score, precision_score, \
...
class Meter(object):
    ...
    def mse(self):
        ...
        for task in range(n_tasks):
            ...
            task_y_pred = y_pred[:, task][task_w != 0].cpu().numpy() 
    ...
    def RBA_score(self):
        """Compute Balanced Accuracy for each task.
        Returns
        -------
        list of float
            Balanced Accuracy for all tasks
        """
        mask = torch.cat(self.mask, dim=0)
        y_pred = torch.cat(self.y_pred, dim=0)
        y_true = torch.cat(self.y_true, dim=0)
        # Convert predictions to binary predictions (0 or 1)
        y_pred = torch.sigmoid(y_pred)
        ...
        for task in range(n_tasks):
            task_w = mask[:, task]
            task_y_true = y_true[:, task][task_w != 0].numpy()
            task_y_pred = y_pred_binary[:, task][task_w != 0].numpy()
            scores.append(balanced_accuracy_score(task_y_true, task_y_pred))
        return scores
    ...
    def precision(self):
        ...
        for task in range(n_tasks):
            ...
            task_y_true = y_true[:, task][task_w != 0].numpy()
            task_y_pred = y_pred_binary[:, task][task_w != 0].numpy()
            scores.append(round(precision_score(task_y_true, task_y_pred, zero_division=0), 4))
        return scores

    def recall(self):
        """Compute Recall (Sensitivity) for each task.
        Returns
        -------
        list of float
        ...
        for task in range(n_tasks):
            ...
            task_y_true = y_true[:, task][task_w != 0].numpy()
            task_y_pred = y_pred_binary[:, task][task_w != 0].numpy()
            scores.append(round(recall_score(task_y_true, task_y_pred, zero_division=0), 4))
        return scores

    def specificity(self):
        """Compute Specificity for each task.
        Returns
        -------
        list of float
            Specificity for all tasks
        """
        mask = torch.cat(self.mask, dim=0)
        y_pred = torch.cat(self.y_pred, dim=0)
        y_true = torch.cat(self.y_true, dim=0)
        ...
        for task in range(n_tasks):
            ...
            tn = np.sum((task_y_true == 0) & (task_y_pred == 0))
            fp = np.sum((task_y_true == 0) & (task_y_pred == 1))
            specificity = tn / (tn + fp) if (tn + fp) > 0 else 0
            scores.append(round(specificity, 4))
        return scores
    ...
    def pr_auc(self):
        """Compute PR AUC (Average Precision) for each task.
        Returns
        -------
        list of float
            PR AUC for all tasks
        """
        mask = torch.cat(self.mask, dim=0)
        y_pred = torch.cat(self.y_pred, dim=0)
        y_true = torch.cat(self.y_true, dim=0)
        ...
        for task in range(n_tasks):
            ...
            task_y_true = y_true[:, task][task_w != 0].numpy()
            task_y_pred = y_pred[:, task][task_w != 0].numpy()
            scores.append(round(average_precision_score(task_y_true, task_y_pred), 4))
        return scores
    ...
    def compute_metric(self, metric_name, reduction='mean'):
        ...
        """
        assert metric_name in ['roc_auc', 'RBA', 'l1', 'rmse', 'mae', 'roc_prc', 'r2', 'return_pred_true', 'mse',
                               'accuracy', 'precision', 'recall', 'specificity', 'f1', 'pr_auc'], \
            'Expect metric name to be in supported metrics, got {}'.format(metric_name)
...
def run_a_train_epoch_heterogeneous(args, epoch, model, data_loader, loss_criterion_c, loss_criterion_r, optimizer, task_weight=None):
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        if args['task_class'] == 'classification_regression':
            ...
            torch.cuda.empty_cache()
    if args['task_class'] == 'classification_regression':
        train_score = np.mean(train_meter_c.compute_metric(args['classification_AROC']) +
                              train_meter_r.compute_metric(args['regression_r2']))
        print('epoch {:d}/{:d}, training {} {:.4f}'.format(
            epoch + 1, args['num_epochs'], 'r2+auc', train_score))
    elif args['task_class'] == 'classification':
        train_score = np.mean(train_meter_c.compute_metric(args['classification_AROC']))
        print('epoch {:d}/{:d}, training {} {:.4f}'.format(
            epoch + 1, args['num_epochs'], args['classification_AROC'], train_score))
    else:
        train_score = np.mean(train_meter_r.compute_metric(args['regression_r2']))
        print('epoch {:d}/{:d}, training {} {:.4f}'.format(
            epoch + 1, args['num_epochs'], args['regression_r2'], train_score))
...
def class_weight(train_set, classification_num):
    """计算每个任务的类别权重（适用于二分类）"""
    # 直接遍历 train_set 中的样本（train_set 是列表）
    task_labels = [sample[2][:classification_num] for sample in train_set]
    task_labels = np.array(task_labels)
    task_masks = [sample[3][:classification_num] for sample in train_set]
    task_masks = np.array(task_masks)
    
    class_weights = []
    for task_idx in range(classification_num):
        valid_labels = task_labels[task_masks[:, task_idx] == 1, task_idx]
        pos_count = (valid_labels == 1).sum()
        neg_count = (valid_labels == 0).sum()
        total = pos_count + neg_count
        
        # 计算类别权重（平衡加权）
        weight_for_0 = total / neg_count if neg_count > 0 else 1.0
        weight_for_1 = total / pos_count if pos_count > 0 else 1.0
        
        class_weights.append([weight_for_0, weight_for_1])
    
    return torch.tensor(class_weights)  # 形状: (num_tasks, 2)
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            if args['task_class'] == 'classification_regression':
                ...
                torch.cuda.empty_cache()
        if args['task_class'] == 'classification_regression':
            return eval_meter_c.compute_metric(args['classification_AROC']) + \
                   eval_meter_r.compute_metric(args['regression_r2'])
        elif args['task_class'] == 'classification':
            return eval_meter_c.compute_metric(args['classification_AROC'])
        else:
            return eval_meter_r.compute_metric(args['regression_r2'])


def run_an_eval_epoch_heterogeneous_r2(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
...
def run_an_eval_epoch_heterogeneous_rmse(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
        ...
        if args['task_class'] == 'classification_regression':
            return eval_meter_c.compute_metric(args['classification_AROC']) + \
                   eval_meter_r.compute_metric(args['regression_rmse'])
        elif args['task_class'] == 'classification':
            return eval_meter_c.compute_metric(args['classification_AROC'])
        else:
            return eval_meter_r.compute_metric(args['regression_rmse'])

def run_an_eval_epoch_heterogeneous_mse(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
        ...
        if args['task_class'] == 'classification_regression':
            return eval_meter_c.compute_metric(args['classification_AROC']) + \
                   eval_meter_r.compute_metric(args['regression_mse'])
        elif args['task_class'] == 'classification':
            return eval_meter_c.compute_metric(args['classification_AROC'])
        else:
            return eval_meter_r.compute_metric(args['regression_mse'])

def run_an_eval_epoch_heterogeneous_mae(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
        ...
        if args['task_class'] == 'classification_regression':
            return eval_meter_c.compute_metric(args['classification_AROC']) + \
                   eval_meter_r.compute_metric(args['regression_mae'])
        elif args['task_class'] == 'classification':
            return eval_meter_c.compute_metric(args['classification_AROC'])
        else:
            return eval_meter_r.compute_metric(args['regression_mae'])

def run_an_eval_epoch_heterogeneous_AROC(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
...
def run_an_eval_epoch_heterogeneous_RBA(args, model, data_loader):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            if args['task_class'] == 'classification_regression':
                ...
                torch.cuda.empty_cache()
        if args['task_class'] == 'classification_regression':
            return eval_meter_c.compute_metric(args['classification_RBA']) + \
                   eval_meter_r.compute_metric(args['regression_mae'])
        elif args['task_class'] == 'classification':
            return eval_meter_c.compute_metric(args['classification_RBA'])
        else:
...
def run_an_eval_epoch_heterogeneous_all_metrics(args, model, data_loader):
    """Evaluate model and return all classification metrics.

    Returns a dictionary with all metrics for each task.
    """
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    ...
    # Compute all metrics
    if args['task_class'] == 'classification' or args['task_class'] == 'classification_regression':
        metrics = {
            'Accuracy': eval_meter_c.accuracy(),
            'Balanced_Accuracy': eval_meter_c.RBA_score(),
            'Precision': eval_meter_c.precision(),
            'Recall': eval_meter_c.recall(),
            'Specificity': eval_meter_c.specificity(),
            'F1_Score': eval_meter_c.f1(),
            'ROC_AUC': eval_meter_c.roc_auc_score(),
            'PR_AUC': eval_meter_c.pr_auc()
        }
    else:
        # For regression tasks, return regression metrics
        metrics = {
            'R2': eval_meter_r.r2(),
            'MSE': eval_meter_r.mse(),
            'MAE': eval_meter_r.mae(),
            'RMSE': eval_meter_r.rmse()
        }
...
def run_an_eval_predmerge(args, model, data_loader, output_path):
    model.eval()
    eval_meter_c = Meter()
    eval_meter_r = Meter()
    smiles_list = [1]
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(data_loader):
            ...
            logitsall = np.concatenate((logitsall, logits))

        result = pd.concat([pd.DataFrame(smiles_list), pd.DataFrame(labelsall), pd.DataFrame(logitsall)],axis=1)        
        result.to_csv(output_path, index=None)
        ...
        for batch_id, batch_data in enumerate(data_loader):
            ...
            for mol_index in range(len(smiles)):
                ...
                if atom_smiles in vis_list:
                    for tasks_index in range(len(args['select_task_list'])):
...
##############################预测##############################
def collate_predict(samples):
    """处理预测数据，每个样本应为(smiles, graph)"""
    # 明确提取前两个元素
    smiles = [s[0] for s in samples]
    graphs = [s[1] for s in samples]
...
def run_predict(args, model, data_loader, output_path):
    """预测函数"""
    model.eval()
    smiles_list = []
    preds_list = []
    ...
    result_df = pd.DataFrame(all_preds, columns=args['select_task_list'])
    result_df.insert(0, 'SMILES', smiles_list)
    result_df.to_csv(output_path, index=False)
    return result_df

def load_pretrained(model_path, args):
    """直接加载模型参数"""
    # 必须完全按照训练时的参数构建模型
    model = MGA(
        in_feats=args['in_feats'],
        rgcn_hidden_feats=args['rgcn_hidden_feats'],
        n_tasks=len(args['select_task_list']),
    ...
    # 直接加载模型状态字典
    state_dict = torch.load(model_path, map_location=torch.device(args['device']))
