...
Updated for Python 3.9: 2025-09-10
Description:
...
Updated for Python 3.9 compatibility with modern package versions
ABBR. used
...
import pandas as pd
import numpy as np
# Updated for Python 3.9: scipy.interp is deprecated, use numpy.interp instead
# from scipy import interp  # Deprecated in scipy 1.9.0+
...
# Updated for Python 3.9: DistanceMetric moved to sklearn.metrics in newer versions
try:
...
class AppDomainX:
    ...
    def hatValue_analyze(self, useXPlus=True):
        ...
        if useXPlus:
            # Updated for Python 3.9: Replace np.matrix with modern numpy arrays
            X = np.column_stack([np.ones(self.dfX.shape[0]), self.dfX.values.astype(float)])
        else:
            X = self.dfX.values.astype(float)
        # hat matrix - using modern numpy operations instead of matrix
        self.hatCoreMatrix = np.linalg.inv(X.T @ X)  # Replace (X.T * X).I
        hatMatrix = X @ self.hatCoreMatrix @ X.T     # Replace X * self.hatCoreMatrix * X.T
    ...
    def hatValue_xenoCheck(self, dfXeno):
        if self.useXPlus:
            # Updated for Python 3.9: Replace np.matrix with modern numpy arrays
            X = np.column_stack([np.ones(dfXeno[self.XLabels].shape[0]), dfXeno[self.XLabels].values.astype(float)])
        else:
    ...
    def LUP_analyze(self,lowerPerc=0, upperPerc=100, PC_Space=False):
        ...
        self.PC_space = PC_Space  # Fixed variable name consistency
        if PC_Space:
            dfX = self.dfX_PC
        else:
            dfX = self.dfX
...
# Additional fingerprint modules
try:
    from rdkit.Avalon import pyAvalonTools
    AVALON_AVAILABLE = True
except ImportError:
    AVALON_AVAILABLE = False
    print("Warning: Avalon fingerprints not available. Please install rdkit-pypi with Avalon support.")
...
try:
    ...
    print("Warning: PubChem fingerprints not available. Please install scikit-fingerprints: pip install scikit-fingerprints")
#
# Wrapper functions for new fingerprint types
def _avalon_fingerprint_wrapper(mol, nBits=512, **kwargs):
    """Wrapper for Avalon fingerprint to match RDKit interface"""
    if not AVALON_AVAILABLE:
        raise ImportError("Avalon fingerprints not available. Please install rdkit-pypi with Avalon support.")

    # Check if mol is None (failed SMILES parsing)
    if mol is None:
        # Return empty fingerprint for invalid molecules
        from rdkit import DataStructs
        fp_bitvect = DataStructs.ExplicitBitVect(nBits)
        return fp_bitvect

    return pyAvalonTools.GetAvalonFP(mol, nBits)

def _maccs_fingerprint_wrapper(mol, **kwargs):
    """Wrapper for MACCS keys fingerprint to handle None molecules"""
    if mol is None:
        # Return empty fingerprint for invalid molecules
        from rdkit import DataStructs
        fp_bitvect = DataStructs.ExplicitBitVect(167)  # MACCS keys length
        return fp_bitvect

    # Use original MACCS keys function
    from rdkit.Chem import MACCSkeys
    return MACCSkeys.GenMACCSKeys(mol)

def _pubchem_fingerprint_wrapper(mol, **kwargs):
    """Wrapper for PubChem fingerprint to match RDKit interface"""
    if not PUBCHEM_AVAILABLE:
        raise ImportError("PubChem fingerprints not available. Please install scikit-fingerprints: pip install scikit-fingerprints")

    # Check if mol is None (failed SMILES parsing)
    if mol is None:
        # Return empty fingerprint for invalid molecules
        from rdkit import DataStructs
        fp_bitvect = DataStructs.ExplicitBitVect(881)  # PubChem fingerprint length
        return fp_bitvect

    # Convert RDKit mol to SMILES for scikit-fingerprints
    from rdkit import Chem
    smiles = Chem.MolToSmiles(mol)

    # Create PubChem fingerprint instance
    fp_generator = PubChemFingerprint()

    # Generate fingerprint for single molecule
    fp_array = fp_generator.transform([smiles])[0]

    # Convert numpy array to RDKit ExplicitBitVect for compatibility
    from rdkit import DataStructs
    fp_bitvect = DataStructs.ExplicitBitVect(len(fp_array))
...
'MACCS_keys':_maccs_fingerprint_wrapper,
'Avalon':_avalon_fingerprint_wrapper,
'PubChem':_pubchem_fingerprint_wrapper
}
#
class AppDomainFpSimilarity:
    ...
    def fpSimilarity_analyze(self, fpType, simiMetric='Tanimoto', **fpOpt): # radius=2, nBits=1024
        ...
        supported fpType:
            ...
            'Avalon': Avalon fingerprint (ExplicitBitVect, default 512 bits, nBits parameter can be specified)
            'PubChem': PubChem fingerprint (ExplicitBitVect, 881 bits, requires scikit-fingerprints package)
...
def mergeYValsList(yValsList):
    ...
    '''
    N = len(yValsList)  # Fixed variable name: yValsA -> yValsList
    ypMerge = np.concatenate([yValsList[i][0] for i in range(N)])
    yscoreMerge = np.concatenate([yValsList[i][1] for i in range(N)])
    ytMerge = np.concatenate([yValsList[i][2] for i in range(N)])
...
# Updated for Python 3.9: Handle graphviz_layout import changes in newer NetworkX versions
try:
    ...
    try:
        ...
        print("Warning: Graphviz layout not available. Using spring layout as fallback.")
...
class NSG:
    ...
    def calcPairwiseSimilarityWithFp(self, fpType, **kw):
        ...
        # MACCS_keys
        # Avalon, nBits=512
        # PubChem (no additional parameters needed)
        self.kw = kw
        self.fpType = fpType
        if fpType == 'Morgan(bit)':
            self.fpTypeStr = 'Morgan(r{:d}n{:d})'.format(kw['radius'],kw['nBits'])
        elif fpType == 'Avalon':
            nBits = kw.get('nBits', 512)
            self.fpTypeStr = 'Avalon(n{:d})'.format(nBits)
        elif fpType == 'PubChem':
            self.fpTypeStr = 'PubChem(881bits)'
        else:
    ...
    def keepCliques(self, minCliqueSize=4, maxISetSize=0):
        ...
        for idx in self.sortedIdx[1:]:  # Fixed variable name: sortedIdx -> self.sortedIdx
            cTmpSet = set(self.cliqueL[idx])  # Fixed variable name: cliqueL -> self.cliqueL
            if len(cTmpSet) < minCliqueSize:
                ...
                kCliquesKept.append(self.cliqueL[idx])  # Fixed variable name: cliqueL -> self.cliqueL
            else: continue
        return kCliquesKept
    def calcComm(self):
...
import matplotlib
# Updated for Python 3.9: Handle optional pyvis dependency
try:
    from pyvis.network import Network
    PYVIS_AVAILABLE = True
except ImportError:
    PYVIS_AVAILABLE = False
    print("Warning: pyvis not available. NSGVisualizer will have limited functionality. Install with: pip install pyvis")
class NSGVisualizer:
    ...
    def calcPos(self, prog='neato', picklePos=True, pickledPosPrefix=''):
        #
        # Updated for Python 3.9: Handle graphviz_layout availability
        if GRAPHVIZ_AVAILABLE:
            self.pos = graphviz_layout(self.nsg.Gss, prog=prog)
        else:
            print(f"Warning: Graphviz not available, using spring layout instead of {prog}")
            self.pos = nx.spring_layout(self.nsg.Gss)
        if picklePos:
