...
for xgb_graph_feats_task in task_list:
    ...
    print('***************************************************************************************************')
    args = {}
    training_set = pd.read_excel(xgb_graph_feats_task+'_training_PubChem.xlsx', index_col=None)
    valid_set = pd.read_excel(xgb_graph_feats_task+'_valid_PubChem.xlsx', index_col=None)
    test_set = pd.read_excel(xgb_graph_feats_task+'_test_PubChem.xlsx', index_col=None)
    ...
    for metric_name in metric_names:
        result_dfs[metric_name].to_csv(xgb_graph_feats_task+'_PubChem_xgb_result_'+metric_name+'.csv', index=None)
    parameters[str(xgb_graph_feats_task)]=args
filename = open('xgb_PubChem_parameters.txt', 'w')
