...
for lgb_graph_feats_task in task_list:
    ...
    print('***************************************************************************************************')
    args = {}
    training_set = pd.read_excel(lgb_graph_feats_task+'_training_ECFP4.xlsx', index_col=None)
    valid_set = pd.read_excel(lgb_graph_feats_task+'_valid_ECFP4.xlsx', index_col=None)
    test_set = pd.read_excel(lgb_graph_feats_task+'_test_ECFP4.xlsx', index_col=None)
    ...
    for metric_name in metric_names:
        result_dfs[metric_name].to_csv(lgb_graph_feats_task+'_ECFP4_lgb_result_'+metric_name+'.csv', index=None)
    parameters[str(lgb_graph_feats_task)]=args
filename = open('lgb_ECFP4_parameters.txt', 'w')
