...
for rf_graph_feats_task in task_list:
    ...
    print('***************************************************************************************************')
    args = {}
    training_set = pd.read_excel(rf_graph_feats_task+'_training_MACCS.xlsx', index_col=None)
    valid_set = pd.read_excel(rf_graph_feats_task+'_valid_MACCS.xlsx', index_col=None)
    test_set = pd.read_excel(rf_graph_feats_task+'_test_MACCS.xlsx', index_col=None)
    ...
    for metric_name in metric_names:
        result_dfs[metric_name].to_csv(rf_graph_feats_task+'_MACCS_rf_result_'+metric_name+'.csv', index=None)
    parameters[str(rf_graph_feats_task)]=args
filename = open('rf_MACCS_parameters.txt', 'w')
